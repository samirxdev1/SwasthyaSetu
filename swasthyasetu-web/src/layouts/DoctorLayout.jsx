 ...
 */
export default function DoctorLayout({ children, activeTab = 'dashboard', setActiveTab }) {
    ...
    <div className="min-h-screen bg-[#F7F6F3] text-[#1C2B2A] font-body flex flex-col md:flex-row">
      ...
      <div className="md:hidden bg-white border-b border-[#1C2B2A]/10 px-4 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded bg-[#1C2B2A]/5 border border-[#1C2B2A]/15 flex items-center justify-center p-1.5 text-[#0F6E5C]">
        ...
        <button
          ...
          className="p-2 rounded hover:bg-[#E7F3EF] text-[#1C2B2A] focus:outline-none focus:ring-2 focus:ring-[#0F6E5C]"
      ...
      >
        <div className="p-4 sm:p-5">
          ...
          <div className="flex items-center gap-3 pb-5 mb-4 border-b border-[#1C2B2A]/10">
            ...
            <div>
              ...
              <span className="font-mono text-[10px] text-[#0F6E5C] font-semibold uppercase tracking-wider block mt-1">
                Doctor Workstation
              </span>
            </div>
          </div>
          ...
          {/* DOCTOR ROLE TAG */}
          <div className="bg-[#E7F3EF] border border-[#0F6E5C]/20 p-2.5 rounded-lg mb-5 flex items-center gap-2">
            ...
            <div className="overflow-hidden">
              ...
              <span className="font-mono text-[10px] text-[#1C2B2A]/70 block truncate">
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-xs font-medium transition-all duration-150 active:scale-98 focus:outline-none focus:ring-2 focus:ring-[#0F6E5C] ${
                    isActive
        ...
        <div className="p-4 border-t border-[#1C2B2A]/10 bg-[#F7F6F3]/50 space-y-3">
          <div className="font-mono text-[10px] text-[#1C2B2A]/60 space-y-1">
          ...
          <button
            onClick={() => navigate('/')}
            className="w-full py-2 px-3 bg-white border border-[#1C2B2A]/20 hover:bg-[#E7F3EF] hover:border-[#0F6E5C] text-[#1C2B2A] text-xs font-mono rounded transition-all duration-150 active:scale-98 flex items-center justify-center gap-1.5 focus:outline-none focus:ring-2 focus:ring-[#0F6E5C]"
          >
            <svg className="w-3.5 h-3.5 text-[#1C2B2A]/70" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-[#1C2B2A]/10 px-4 sm:px-6 py-3.5 flex items-center justify-between sticky top-0 z-10 shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden sm:inline-block font-mono text-xs px-2.5 py-0.5 bg-[#E7F3EF] text-[#0F6E5C] rounded font-semibold border border-[#0F6E5C]/20">
              CLINICAL V2.6
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                className="relative p-2 rounded-lg text-[#1C2B2A]/70 hover:bg-[#E7F3EF] hover:text-[#0F6E5C] transition-colors focus:outline-none focus:ring-2 focus:ring-[#0F6E5C]"
                aria-label="View notifications"
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                </svg>
                <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-[#C9754A] ring-2 ring-white"></span>
              </button>

              {/* NOTIFICATIONS POPOVER CARD */}
              {notificationsOpen && (
                <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white border border-[#1C2B2A]/15 rounded-xl shadow-lg p-3.5 z-40 animate-entrance">
                  <div className="flex items-center justify-between pb-2 mb-2 border-b border-[#1C2B2A]/10">
                    <span className="font-display font-bold text-xs text-[#1C2B2A] uppercase tracking-wider">
                      Clinical Alerts &amp; Notifications
                    </span>
                    <span className="font-mono text-[10px] text-[#0F6E5C] bg-[#E7F3EF] px-1.5 py-0.5 rounded font-medium">
                      2 Unread
                    </span>
                  </div>
                    ...
                    {notifications.map((n) => (
                      ...
                      >
                        <div className="flex justify-between font-semibold text-[#1C2B2A]">
                          <span>{n.title}</span>
                          <span className="font-mono text-[10px] text-[#1C2B2A]/50">{n.time}</span>
                        </div>
                        <p className="text-[11px] text-[#1C2B2A]/70 mt-1 leading-snug">{n.desc}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {/* DOCTOR QUICK BADGE */}
            <div className="hidden sm:flex items-center gap-2 border-l border-[#1C2B2A]/10 pl-3">
              <div className="w-7 h-7 rounded-full bg-[#0F6E5C] text-white flex items-center justify-center font-display text-xs font-bold">
                AS
              </div>
              <div className="text-left leading-none">
                <span className="text-xs font-semibold text-[#1C2B2A] block">Dr. A. Sharma</span>
                <span className="font-mono text-[10px] text-[#1C2B2A]/60 block">Cardiology OPD</span>
              </div>
            </div>
