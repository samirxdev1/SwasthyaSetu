  ...
  return (
    <div className="bg-white border border-[#E7F3EF] rounded-xl p-5 sm:p-6 shadow-sm space-y-6">
      ...
      <div className="flex items-center justify-between border-b border-[#1C2B2A]/10 pb-4">
        ...
        <span className="font-mono text-xs text-[#3B7A9E] bg-[#3B7A9E]/10 border border-[#3B7A9E]/20 px-2.5 py-1 rounded font-semibold">
          SIGNAL BLUE LAB HUB
        </span>
      </div>

      {/* NEW LAB ORDER DISPATCH FORM */}
      <form onSubmit={handleSubmit} className="bg-[#F7F6F3] p-4 rounded-xl border border-[#1C2B2A]/10 space-y-4">
        <span className="font-mono text-xs font-semibold text-[#1C2B2A] uppercase tracking-wider block">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {/* TEST SELECTION */}
          <div>
            <label htmlFor="lab-test-select" className="block text-xs font-mono font-semibold uppercase text-[#1C2B2A] mb-1">
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-white border border-[#1C2B2A]/20 rounded-lg text-xs font-mono text-[#1C2B2A] focus:outline-none focus:border-[#3B7A9E] focus:ring-2 focus:ring-[#E7F3EF]"
            >
          ...
          <div>
            <label htmlFor="lab-facility-select" className="block text-xs font-mono font-semibold uppercase text-[#1C2B2A] mb-1">
          ...
          {/* PRIORITY SELECTION */}
          <div>
            <label htmlFor="lab-priority-select" className="block text-xs font-mono font-semibold uppercase text-[#1C2B2A] mb-1">
            ...
            <div className="flex gap-2">
              {['Routine', 'Urgent', 'STAT'].map((p) => (
                <button
                  ...
                  className={`flex-1 py-1.5 px-2 font-mono text-xs rounded-lg transition-all border ${
                    priority === p
                      ? p === 'STAT'
                        ? 'bg-[#C9754A] text-white border-[#C9754A] font-bold'
                        : 'bg-[#3B7A9E] text-white border-[#3B7A9E] font-bold'
                      : 'bg-white text-[#1C2B2A]/70 border-[#1C2B2A]/20 hover:border-[#3B7A9E]'
                  }`}
                >
                  {p}
                </button>
          ...
          <div>
            <label htmlFor="lab-notes" className="block text-xs font-mono font-semibold uppercase text-[#1C2B2A] mb-1">
            ...
            <input
              ...
              className="w-full px-3 py-1.5 bg-white border border-[#1C2B2A]/20 rounded-lg text-xs text-[#1C2B2A] focus:outline-none focus:border-[#3B7A9E]"
            />
          </div>
        ...
        <div className="flex justify-end pt-1">
          <button
            type="submit"
            className="py-2.5 px-5 bg-[#3B7A9E] hover:bg-[#316583] text-white font-mono text-xs font-semibold rounded-lg transition-all duration-150 active:scale-98 focus:outline-none focus:ring-2 focus:ring-[#3B7A9E] flex items-center gap-1.5"
          >
      ...
      <div className="space-y-3 pt-2">
        <div className="flex items-center justify-between">
          <h3 className="font-display font-bold text-sm sm:text-base text-[#1C2B2A]">
        ...
        <div className="space-y-2">
          {orders.map((ord) => {
            ...
            // Pending -> Warm Fog / Ink Slate
            // In Progress -> Signal Blue (#3B7A9E)
            // Report Ready -> Deep Teal (#0F6E5C)
            ...
            return (
              <div
                key={ord.id}
                className="p-3.5 bg-white border border-[#1C2B2A]/10 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-[#3B7A9E]/40 transition-colors"
              >
                <div className="space-y-1">
                  ...
                  <div className="text-xs text-[#1C2B2A]/70 flex items-center gap-2">
                    <span>Patient: <strong>{ord.patientName}</strong></span>
                    <span className="font-mono text-[10px] text-[#0F6E5C]">({ord.healthId})</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-mono border transition-colors duration-200 ease-out cursor-pointer active:scale-98 flex items-center gap-1.5 ${chipStyle}`}
                >
